...
```{r}
Bank %>% select_if(is.character) %>% dplyr::select(-y) %>% head(3)
```
...
```{r}
BankN <- Bank %>% dummy_cols(select_columns = c("job", "marital", "education", "default", "housing", "loan", "contact", "y"), remove_first_dummy = TRUE) %>% dplyr::select(-c("job", "marital", "education", "default", "housing", "loan", "contact","y")) %>% as.data.frame()
...
```{r}
BankN %>% dplyr::select(starts_with("job")) %>% as.matrix() %>% head()
```
...
```{r}
BankN <- BankN %>% mutate(contacts = campaign + previous) %>% dplyr::select(-c("campaign","previous", "pdays"))
```

The *day* column and the *month* column are linked, so we can remove the *day* column all together. 

```{r}
BankN <- BankN %>% dplyr::select(-day)
```
