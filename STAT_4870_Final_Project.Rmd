...
rtrainx <- train %>% dplyr::select(-y, age, job, balance, month, duration, calls) |> dummy_cols( remove_first_dummy = TRUE, remove_selected_columns = TRUE)
rtrainy <- train %>% pull(y)
rtestx <- test %>% dplyr::select(-y, age, job, balance, month, duration, calls) |> dummy_cols( remove_first_dummy = TRUE, remove_selected_columns = TRUE)
rtesty <- test %>% pull(y)
...
mean(ryhat == rtesty) #85.027 accuracy #84.349 accuracy
```
...
mean(ryhat == rtesty) #91.62 accuracy #91.27
```
