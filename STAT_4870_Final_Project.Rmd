...
The "unknown" variable is undoubtebly an outlier since it has a small count compared to the 40,000+ dataset. Let's remove it and look at the following bar plot:
...
```{r}
attach(BankN)
plot(x = tapply(y, age, var), type = "l", xlab = "Age Variance")

plot(x = tapply(y, age, mean), type = "l", xlab = "Age mean")
...
```{r}
jobM <- BankN %>% select(starts_with("job")) %>% as.matrix()
...
```{r}
fit <- lm(BankN$y ~ BankN$age)
r = fit$residuals
f = fitted(fit)
plot(f, r, pch = 18, xlab = "Fitted Values", ylab = "Residuals") + abline(h = 0, col = "red", lwd = 2)
```

```{r}
#BBIC <- step(fit,direction="backward",scope=list(lower=~Public,upper=fit), data=X)
```
